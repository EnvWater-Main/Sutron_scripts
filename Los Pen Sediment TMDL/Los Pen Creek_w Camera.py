...
imageFileName = "LosPenCreek_{YY}{MM}{DD}{hh}{mm}{ss}.jpg"
...
overlayFontSize = 24   # font height for overlay text
...
def turn_on_sampling():
    ...
    ## change sensor logging intervals to 1 minute (on 5 min during non-sampling periods)
    ## changes measurements M5 through M14 to 1min interval
    for i in [i for i in range(5,14,1)]:
        setup_write("!M"+str(i)+" meas interval", "00:01:00")
        
    ## Change camera frequency
    setup_write("!S5 scheduled interval","00:15:00") ## during sampling, take a photo every 15 min interval
    setup_write("!TX2 scheduled interval","00:15:00") ## during sampling, send the photo every 15 min interval
    setup_write("!TX2 scheduled time","00:01:00") # send the photo 1 minute after the 15 min interval
...
def turn_off_sampling():
    ...
    ## Change camera frequency
    setup_write("!S5 scheduled interval","04:00:00") ## during non-sampling, take a photo every 4 hr interval
    setup_write("!TX2 scheduled interval","04:00:00") ## during non-sampling, send the photo every 4 hr interval
    setup_write("!TX2 scheduled time","00:01:00") # send the photo 1 minute after the 4hr interval
