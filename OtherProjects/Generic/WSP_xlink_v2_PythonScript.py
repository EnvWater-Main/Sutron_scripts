...
def gp_find_index_from_label(label):
...
def get_flow_units():
    ...
    pacing_vol_from_flow_units = {'cfs': 'cf', 'gpm': 'gal', 'm3s': 'm3'}
    flow_pacing_units = pacing_vol_from_flow_units.get(flow_pacing_data_source_units, '')
...
## Pacing Weighting
pacing_weighting = get_pacing_weighting() # "FLOW" or "TIME"

## Time Pacing Increment - if it's TIME get the pacing increment, ie how many minutes to add between measurement cycles
time_pacing_increment = get_time_pacing_increment()

## Flow Units
flow_pacing_data_source_units, flow_pacing_units = get_flow_units()

# Sample pacing for TIME and FLOW -if TIME the GP value is minutes, if FLOW the GP values is in whatever flow units specified - keep a global one to check if there are any changes
...
@MEASUREMENT
def rain_1_min(inval):
    """
    Computes the rainfall during the last 60 seconds.
        Another measurement labeled Rain_950 must be recording precip accumulation.
    """
    return differential_reading("Rain_950", 60, False)  # 60 sec = 1 minute.  False means no negative readings.
