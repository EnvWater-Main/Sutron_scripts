...
@MEASUREMENT
def CompoundWeir_cfs(level_in):
    ## screen negative values
    if level_in <= 0:
        level_in = 0.0
    
    ## Get weir dims from field measurements
    vnotch_in= 4.0
    b2 = 8.0
    b1 = 3.5 + 3.5
    c1 = 1.25
    ...
    h2_m = level_in * 0.0254 ## Height above V
    if h2_m > 0:    
        h2_eff_m =  h2_m + 0.0008 ## Effective head
    else:
        h2_eff_m = 0
        
    h1_m = h2_m - vnotch_m ## Height above Horizontal crest
    if h1_m > 0:    
        h1_eff_m =  h1_m + 0.0008 ## Effective head
    else:
        h1_eff_m = 0
    ...
    Flow_m3s_ctrsc =     ((8./15.) * Ctd * ((2.*9.81)**0.5) * (math.tan(math.radians(90.)/2.)) * (h2_eff_m**2.5 - h1_eff_m**2.5))    +     ((2./3.) * Crd * ((2.*9.81)**0.5) * (2 * b1_m) * (h1_eff_m**1.5))
    ## change from m3/s to gpm
    #Flow_gpm_ctrsc = Flow_m3s_ctrsc * 15850.372483753
    Flow_cfs_ctrsc = Flow_m3s_ctrsc * 35.314666212661

    return Flow_cfs_ctrsc
