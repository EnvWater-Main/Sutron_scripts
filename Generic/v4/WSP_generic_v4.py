...
aliquot_vol_mL = int(gp_read_value_by_label("aliquot_vol_mL"))
vol_in_bottle = 0.0
...
def modbus_crc16(data: bytes) -> bytes:
    """Compute Modbus RTU CRC16 checksum."""
    crc = 0xFFFF
    for byte in data:
        crc ^= byte
        for _ in range(8):
            if crc & 0x0001:
                ...
                crc >>= 1
    # Return low byte first, then high byte (Modbus order)
    return struct.pack('<H', crc)

def build_frame(value: int) -> bytes:
    """
    Build a Modbus RTU frame like:
    b'\x02\x10\x26\xCF\x00\x02\x04\x00\xFA\x80\x07\xA9\x02'
        - The 00 FA bytes will be replaced with the chosen integer.
    - CRC16 will be recalculated automatically.
    """
    # Frame prefix up to where value is inserted
    prefix = b'\x02\x10\x26\xCF\x00\x02\x04'
        # Replace x00\xFA with your integer (2 bytes, big-endian)
    value_bytes = struct.pack('>H', value)
        # The rest of the static bytes (after 00 FA)
    suffix = b'\x80\x07'
        # Combine without CRC
    frame_wo_crc = prefix + value_bytes + suffix
        # Compute CRC
    crc = modbus_crc16(frame_wo_crc)
        # Full frame with CRC appended
...
def grab_sample_modbus():
    ...
    grab_sample = build_frame(aliquot_vol_mL) ## build bytestring to send modbus message on aliquot volume for grab sample
...
def trigger_sampler():
    ...
    if trigger == True:
        ...
        ## TRIGGER Sampler
        pow_or_modbus = gp_read_value_by_label(sampl_aux_or_modbus)
        if pow_or_modbus == 0:
            # trigger sampler by pulsing output for 0.5 seconds
            #  if using Switched Power port
            power_control('SW1', True)
            utime.sleep(0.5)
            power_control('SW1', False)
            
            # if using Digital Output port. DOUT appears to work counter-intuitively, where "on" means no voltage, and "off" means voltage
            # we may need to be using a 10 kOhm resistor
